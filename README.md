...
Окно VoiceType открывается ярлыком **VoiceType Local** на рабочем столе или
обычным нажатием на его значок возле часов. Правый клик по значку оставляет
полное меню с пунктом **«Настройки и приватность…»**. Автозагрузка запускает
только фоновый tray-режим; последующий запуск ярлыка поднимает то же окно.
...
- Unicode-вставка без изменения clipboard: одно атомарное обращение к
  `SendInput`, затем точная UI Automation-проверка фактического изменения поля;
...
- однозначные замены только по целым словам с защитой чисел, дат, сумм, URL,
  email и кодов;
- локальная бережная коррекция использует только текущую фразу и небольшой
  набор точных контекстных правил; скрытая история диктовок для неё не ведётся;
...
## Текущий кандидат: VoiceType Control
...
- **Управление** — речь разбирается только строгим RU/ES/EN-парсером;
- **Обычный** — речь остаётся диктовкой, Windows-команда принимается только
  после точного слова `команда`, `command` или `comando`. Дополнительно можно
  закончить непустой текст точной фразой `нажать Enter` / `press Enter` /
  `pulsa Intro` либо `отправить сообщение` / `send message` /
  `enviar mensaje`.

Конечная команда вырезается из печатаемого текста. Сначала VoiceType проверяет
фокус и успешно вставляет текст, затем просит голосовое подтверждение и только
после него нажимает Enter. При смене окна, ошибке или частичной вставке действие
не выполняется.
...
Добавлено безопасное редактирование **только последней вставки VoiceType в том
же поле**: удалить последнее слово/предложение/всю вставку, заменить последнее
слово/предложение, выполнить однозначную точную замену и вернуть последнее
исправление. Перед изменением приложение повторно проверяет окно, поле, курсор,
границы и исходный текст. При неоднозначности или смене фокуса оно ничего не
удаляет. В режиме `Обычный` нужен префикс `команда / command / comando`, в
`Управление` — нет, а в `Диктовка` эти фразы всегда печатаются как текст.

### Быстрая проверка

В **Настройках** можно открыть отдельное собственное тестовое поле и пройти
четыре голосовых шага без возвращения в чат: продиктовать контрольную фразу,
удалить последнее предложение, вернуть исправление и заменить последнее
предложение. На время этой проверки VoiceType
принудительно использует локальный Whisper и локальную коррекцию — облако не
вызывается. Тестовый текст и результаты шагов существуют только в RAM, не
добавляются в историю и технические логи. Сводка не содержит транскрипт и
копируется в буфер обмена только по явному нажатию пользователя.

Это быстрый smoke-тест основной цепочки, а не замена выборочной приёмки в
реальных приложениях: Notepad, браузере, мессенджерах и других целевых полях.
...
- [замеры задержки и принятые оптимизации](docs/PERFORMANCE_AUDIT_2026-07-18.md);
- [baseline security-аудит](docs/SECURITY_AUDIT_2026-07-17.md).

Постоянная лента истории в эту версию намеренно не входит.
Команда «выделить последнее предложение» также отложена и в текущем интерфейсе
не показывается.
...
Установщик назначает ярлыкам разные безопасные роли, не меняя сохранённые
настройки пользователя: ярлык на рабочем столе открывает окно VoiceType Local,
а ярлык в автозагрузке передаёт `--background` и запускает программу только в
трее. Если фоновый экземпляр уже работает, запуск ярлыка рабочего стола должен
показать его существующее окно, не создавая второй обработчик горячих клавиш.
...
`windows_control`, `voice_control`, `voice_commands`, `dictation_transform`,
`session_editing`, `session_editor` и `windows_session_editor_probe`.
Packaged `--self-test` повторяет эту проверку до загрузки модели и запускает
реальный изолированный Windows RichEdit-пробник. Пробник создаёт собственное
невидимое поле и не читает и не изменяет пользовательские окна или документы.
